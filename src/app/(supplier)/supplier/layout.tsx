...
const supplierLinks = [
  ...
  { href: "/supplier/ventas", label: "Estadísticas" },
  { href: "/supplier/inventario", label: "Inventario" },
];
...
export default function SupplierLayout({
  ...
  const getPageTitle = () => {
    const current = supplierLinks.find((l) => l.href === pathname);
    return current?.label || "PROVEEDOR";
  };

  return (
    <div className="flex min-h-screen">
      <aside className="hidden md:block w-64 bg-black text-white p-4 flex flex-col fixed h-screen">
        <h1 className="text-xl font-black mb-8">PROVEEDOR</h1>
